...
def recommend_movies(movie):
    ...
    temp = temp.split()
    if len(temp) > 1:
        if temp[0] in ['A', 'An', 'The', 'Mr.', 'Saving'] and len(temp) == 2:
            temp = temp[1]
        elif len(temp) > 2 and temp[0] not in ['A', 'An', 'The', 'Mr.', 'Saving']:
            temp = temp[0]
        else:
            temp = ' '.join(temp)
    else:
...
def resize_img(photo, x, y):
    ...
    try:
        ...
        r = requests.get("https://st3.depositphotos.com/23594922/31822/v/600/depositphotos_318221368-stock-illustration-missing-picture-page-for-website.jpg")
        img = Image.open(BytesIO(r.content))
        resizedImg = img.resize((x, y), Image.ANTIALIAS)
        return resizedImg
                    ...
                    for iteration in range(len(recommended[0])):
                        ...
                        with cols[iteration % 4]:
                            ...
                            with st.expander(label=str("See Details"), expanded=False):
                                ...
                                st.write(f"**{omdb_dict.get('Title', 'N/A')}**")

                                st.write("**Directed by:** " + omdb_dict.get("Director", "N/A"))
                                st.write("**Overview:** " + omdb_dict.get("Plot", "N/A"))
